...
Object.assign(Runtime.Monad.prototype,
{
	...
	call: function(f)
	{
		...
		try
		{
			this.val = Runtime.rtl.apply(f, Runtime.Vector.from([this.val]));
		}
		catch (_ex)
		{
			if (true)
	...
	callAsync: async function(f)
	{
		...
		try
		{
			this.val = await Runtime.rtl.apply(f, Runtime.Vector.from([this.val]));
		}
		catch (_ex)
		{
			if (true)
...
Object.assign(Runtime.Callback.prototype,
{
	...
	{
		if (args == undefined) args = null;
		return Runtime.rtl.apply(this, args);
	},
	_init: function()
	{
		this.obj = null;
...
Object.assign(Runtime.Web.BaseModel.prototype,
{
	...
	{
		this.listeners.set(message_name, Runtime.Vector.from([]));
	},
	/**
	 * Add listener
	 */
	addListener: function(message_name, f)
	{
		...
		{
			this.listeners.get(message_name).append(f);
		}
	},
	/**
	 * Emit message
	...
	emitMessage: function(message_name, message)
	{
		...
		var listeners = this.listeners.get(message_name);
		for (var i = 0; i < listeners.count(); i++)
		{
			var f = listeners.get(i);
			Runtime.rtl.apply(f, Runtime.Vector.from([message]));
		}
	},
	/**
	 * Load data
...
Object.assign(Runtime.Web.BaseLayoutModel.prototype,
{
	...
	{
		return Runtime.rs.removeFirstSlash(path);
	},
	/**
	 * Returns url
	 */
...
Object.assign(Runtime.Web.BusHttp.prototype,
{
	...
	{
		var __v0 = new Runtime.Monad(Runtime.rtl.attr(params, "service"));
		__v0 = __v0.monad(Runtime.rtl.m_to("string", "app"));
		var service = __v0.value();
		var __v1 = new Runtime.Monad(Runtime.rtl.attr(params, "api_name"));
		__v1 = __v1.monad(Runtime.rtl.m_to("string", ""));
		var api_name = __v1.value();
		var __v2 = new Runtime.Monad(Runtime.rtl.attr(params, "method_name"));
		__v2 = __v2.monad(Runtime.rtl.m_to("string", ""));
		var method_name = __v2.value();
		var __v3 = new Runtime.Monad(Runtime.rtl.attr(params, "data"));
		__v3 = __v3.monad(Runtime.rtl.m_to("Runtime.Dict", null));
		var data = __v3.value();
		...
		try
		{
			...
			var json_data = Runtime.rtl.json_encode(data);
...
Object.assign(Runtime.Web.RenderContainer.prototype,
{
	...
	createLayout: function()
	{
		...
		/* Create layout */
		this.layout = Runtime.rtl.newInstance(params.get("class_name"), Runtime.Vector.from([Runtime.Map.from({"backend_storage":this.backend_storage,"route":this.route})]));
	...
	importData: function(data)
	{
		...
		serializer.setFlag(Runtime.Serializer.ALLOW_OBJECTS);
		/* Get route */
		this.route = serializer.decode(data.get("layout").get("route"));
		/* Create layout */
		this.createLayout();
		/* Load data */
...
Object.assign(Runtime.Web.RouteInfo.prototype,
{
	...
	_changes: function(changes)
	{
		if (changes.has("uri") && !changes.has("uri_match"))
		{
			var uri = Runtime.rtl.attr(changes, "uri");
			var uri_match = Runtime.rtl.attr(changes, "uri_match");
			...
			{
				changes.set("params", Runtime.Vector.from([]));
			}
			changes.set("uri_match", "^" + Runtime.rtl.toStr(uri_match) + Runtime.rtl.toStr("$"));
		}
	},
	/**
	 * Add matches
	...
	{
		var new_matches = Runtime.Map.from({});
		...
		return this.clone(Runtime.Map.from({"matches":new_matches}));
	},
	/**
	 * Resolve route
	 */
	resolve: async function(container)
	{
		...
		if (this.action != null)
		{
			...
			{
				var base_route = this.action.factory();
				var action = new Runtime.Callback(base_route, base_route.action);
				if (action.exists())
				{
		...
		if (page_model_name != "" && Runtime.rtl.class_exists(page_model_name))
		{
	...
	_init: function()
	{
		...
		this.layout = null;
		this.data = null;
		this.middleware = Runtime.Vector.from([]);
	...
	takeValue: function(k,d)
	{
		...
		else if (k == "is_backend")return this.is_backend;
...
{
	Runtime.Entity.Hook.call(this, "Runtime.Web.Hooks.SetupLayout", params);
};
...
Object.assign(Runtime.Web.Hooks.Components.prototype,
{
	...
	_init: function()
	{
		...
		this.header = Runtime.Vector.from([]);
		this.footer = Runtime.Vector.from([]);
	},
});
...
Object.assign(Runtime.Web.Hooks.SetupLayout.prototype,
{
	...
	register_hooks: function()
	{
		this.register(this.constructor.COMPONENTS, "registerComponents");
		this.register(this.constructor.LAYOUT_COMPONENT_NAME);
		this.register(this.constructor.LAYOUT_MODEL_NAME);
	},
	/**
	 * Components
	 */
	registerComponents: function(params)
	{
		Runtime.rtl.attr(params, "components").appendItems(this.components);
	},
	/**
	 * Layout model name
	 */
	layout_model_name: function(params)
	{
		...
		if (this.names && this.names.has(layout_name) && this.names.get(layout_name).has("model"))
		{
	...
	{
		var layout_name = params.get("layout_name");
		if (this.names && this.names.has(layout_name) && this.names.get(layout_name).has("component"))
		{
...
Object.assign(Runtime.Web.ModuleDescription,
{
	...
	entities: function()
	{
		return Runtime.Vector.from([new Runtime.Entity.Provider("Runtime.Web.RenderProvider")]);
	},
	/* ======================= Class Init Functions ======================= */
	getNamespace: function()
	{
...
Runtime.Widget.Input = {
	...
	methods:
	{
		render: function()
		{
			...
			/* Element 'input' */
			let __v0 = this._e(__v, "input", this._merge_attrs({"type":this.type,"name":this.name,"value":this.value,"placeholder":this.placeholder,"ref":"input","onChange":this.onChange,"onKeydown":this.onKeyDown,"class":this._class_name(["widget_input"])}, props));
...
Runtime.Widget.Link = {
	...
	methods:
	{
		render: function()
		{
			...
			/* Element 'a' */
			let __v0 = this._e(__v, "a", {"href":this.href,"target":this.target,"class":this._class_name([this.class])});
	...
	methods:
	{
		render: function()
		{
			...
			if (this.pages > 1)
			{
				...
				if (this.name)
				{
					Runtime.rtl.getContext().translate = props.set("href", this.getPageUrl(1));
				}
				...
				if (page_start <= page_end)
				{
					for (let p = page_start; p <= page_end; p++)
					{
						if (this.name)
						{
							Runtime.rtl.getContext().translate = props.set("href", this.getPageUrl(p));
						}
				...
				if (this.pages > 1)
				{
					if (this.name)
					{
						Runtime.rtl.getContext().translate = props.set("href", this.getPageUrl(this.pages));
					}
...
Runtime.Widget.Container = {
	...
	methods:
	{
		render: function()
		{
			...
			/* Element 'div' */
			let __v0 = this._e(__v, "div", {"style":this.getStyle(),"class":this._class_name(["widget_container", this.getClass()])});
 ...
 */
		getClass: function()
		{
			...
			if (this.flex == "true")
			{
				styles.push("widget_container--flex");
			}
 ...
 */
	getDefaultTemplate: function()
	{
		...
		{
			return Runtime.Map.from({"template":Runtime.rs.join("\n", Runtime.Vector.from(["<style>","padding-top: 10px;","padding-bottom: 10px;","padding-left: 0px;","padding-right: 0px;","</style>"]))});
		}});
	},
	css: function(vars)
	{
		var res = "";
		res += Runtime.rtl.toStr(".widget_container--flex.h-414b{display: flex}");
		return res;
	},
	getNamespace: function()
	{
...
Object.assign(Runtime.Widget.RowButtonsModel.prototype,
{
	 ...
	 * Add button
	 */
	addButton: function(button, dest, kind)
	{
		if (dest == undefined) dest = "";
		if (kind == undefined) kind = "after";
		/* Add button */
		var model = this.addWidget("Runtime.Widget.ButtonModel", button);
 ...
 */
		getStyle: function()
		{
			...
			if (this.image)
			{
				res.push("background-image: url(" + Runtime.rtl.toStr(this.layout.assets(this.image)) + Runtime.rtl.toStr(")"));
			}
			return Runtime.rs.join(";", res);
		},
		/**
		...
		render: function()
		{
			...
			if (options != null)
			{
				for (let i = 0; i < options.count(); i++)
				{
					...
					if (item.get("key") == this.value && this.value !== "" && this.value !== null)
					{
						selected = Runtime.Map.from({"selected":"selected"});
					}
...
Object.assign(Runtime.Widget.Select,
{
	...
	{
		var res = "";
		res += Runtime.rtl.toStr(".widget_select.h-d72d,.widget_select.h-d72d:focus{width: 100%;font-family: var(--widget-font-family);font-size: var(--widget-font-size);padding: var(--widget-button-padding-y) var(--widget-button-padding-x);background-color: var(--widget-color-default);border-width: var(--widget-border-width);border-color: var(--widget-color-border);border-style: solid;border-radius: 4px;box-shadow: none;outline: transparent;line-height: normal;color: inherit;max-width: 100%;min-height: 32px}.widget_select.h-d72d:hover{color: inherit}");
		return res;
	},
	getNamespace: function()
	{
...
Runtime.Widget.SelectLabel = {
	...
	methods:
	{
		render: function()
		{
			...
			{
				return item.get("value") == this.value;
			})) : (null);
			
			/* Render */
			this._t(__v0, (item != null) ? (item.get("label")) : (this.value));
...
Runtime.Widget.Text = {
	...
	methods:
	{
		renderContent: function()
		{
			...
			{
				/* Raw */
				this._t(__v, new Runtime.RawString(this.content));
			}
			else
			{
				/* Render */
				this._t(__v, this.content);
			}
...
Runtime.Widget.TextImage = {
	...
	methods:
	{
		render: function()
		{
			...
			/* Render */
			this._t(__v1, this.renderSlot("image"));
			
			/* Element 'div' */
			let __v2 = this._e(__v0, "div", {"class":this._class_name(["widget_text_image__text"])});
			
			/* Render */
			this._t(__v2, this.renderSlot("text"));
 ...
 */
		getClass: function()
		{
			var styles = this.styles.map((item) =>
			{
				return "widget_text_image--" + Runtime.rtl.toStr(item);
			});
 ...
 */
	getDefaultTemplate: function()
	{
		return Runtime.Map.from({"default":() =>
		{
			return Runtime.Map.from({"template":Runtime.rs.join("\n", Runtime.Vector.from(["<use name='Runtime.Widget.Image' component='true' />","<use name='Runtime.Widget.Text' component='true' />","<style>","%(TextImage)widget_text_image{","  &__image{","  }","  &__text{","  }","}","</style>","<template>","\t<slot name='image'>","\t\t<Image class='[&_image]' />","\t</slot>","\t<slot name='text'>","\t\t<Text class='[&_text] center' content='Image text' />","\t</slot>","</template>"]))});
		}});
	},
	css: function(vars)
	{
		var res = "";
		res += Runtime.rtl.toStr(".widget_text_image--text_right.h-e2c7{display: flex;align-items: start;justify-content: space-between}");
		return res;
	},
	getNamespace: function()
	{
	...
	getFieldInfoByName: function(field_name)
	{
		...
		if (field_name == "kind") return Map.from({
			"t": "string",
			"annotations": Vector.from([
				new Runtime.Web.Annotations.Param(Runtime.Map.from({"label":"Kind","name":"Runtime.Widget.Select","props":Runtime.Map.from({"options":Runtime.Vector.from([Runtime.Map.from({"label":"Text right","value":"text_right"}),Runtime.Map.from({"label":"Text left","value":"text_left"}),Runtime.Map.from({"label":"Text top","value":"text_top"}),Runtime.Map.from({"label":"Text bottom","value":"text_bottom"})])})})),
			]),
		});
		if (field_name == "styles") return Map.from({
			"t": "Runtime.Collection",
			"s": ["string"],
			"annotations": Vector.from([
				new Runtime.Web.Annotations.Param(Runtime.Map.from({"label":"Styles","name":"Runtime.Widget.Tag"})),
			]),
		});
		return null;
	},
...
Runtime.Widget.WidgetResult = {
	...
	methods:
	{
		render: function()
		{
			...
			/* Element 'div' */
			let __v0 = this._e(__v, "div", {"class":this._class_name(["widget_result", this.getErrorClass()])});
...
Object.assign(Runtime.Widget.WidgetResult,
{
	...
	{
		var res = "";
		res += Runtime.rtl.toStr(".widget_result.h-e870{text-align: center;margin-top: var(--widget-space)}.widget_result--success.h-e870{color: var(--widget-color-success)}.widget_result--error.h-e870{color: var(--widget-color-danger)}.widget_result--hide.h-e870{display: none}");
		return res;
	},
	getNamespace: function()
	{
...
Object.assign(Runtime.Widget.Dialog.DialogModel.prototype,
{
	...
	initWidget: function(params)
	{
		...
		/* Add result */
		this.result = this.addWidget("Runtime.Widget.WidgetResultModel");
	},
	/**
	 * Set title
	 */
...
Object.assign(Runtime.Widget.Dialog.ConfirmDialogModel.prototype,
{
	...
	onConfirmButtonClick: async function()
	{
		try
		{
			...
			this.emit(new Runtime.Widget.Dialog.DialogMessage(Runtime.Map.from({"name":"confirm"})));
		}
		catch (_ex)
		{
...
Runtime.Widget.Form.Form = {
	...
	methods:
	{
		...
		renderField: function(field)
		{
			...
			let value = this.model.getItemValue(field_name);
			...
			if (field_component != null)
			{
				...
				},"data":Runtime.Map.from({"field_name":field_name,"form":this.model})}, field_props));
			}
			else
			{
				/* Render */
				this._t(__v, this.renderWidget(field_model, Runtime.Map.from({"name":field_name,"value":value,"ref":"field_" + Runtime.rtl.toStr(field_name),"data":Runtime.Map.from({"field_name":field_name,"form":this.model,"value":value})})));
			}
...
Object.assign(Runtime.Widget.Form.FormModel.prototype,
{
	...
	submit: async function()
	{
		...
		this.emit(new Runtime.Widget.Form.FormMessage(Runtime.Map.from({"name":"submit","result":res})));
		return Promise.resolve(res);
	},
	_init: function()
	{
...
Object.assign(Runtime.Widget.Form.FormSaveStorage.prototype,
{
	...
	load: async function()
	{
		...
		var res = await this.form.layout.callApi(Runtime.Map.from({"api_name":this.getApiName(),"method_name":"actionItem","data":post_data}));
		return Promise.resolve(res);
	},
	/**
	 * Submit form
	...
	submit: async function()
	{
		...
		var res = await this.form.layout.callApi(Runtime.Map.from({"api_name":this.getApiName(),"method_name":"actionSave","data":post_data}));
		return Promise.resolve(res);
	},
	_init: function()
	{
...
Runtime.Widget.Gallery.Gallery = {
	...
	{
		return {
			dialog: new Runtime.Widget.Gallery.GalleryDialogModel(Runtime.Map.from({"modal":false})),
		};
	},
	methods:
	{
		renderItem: function(pos)
		{
			...
			/* Component '{component}' */
			let __v0 = this._c(__v, component, {"item":this.items.get(pos),"onClick":() =>
			{
				this.onClick(pos);
			}});
		...
		render: function()
		{
			...
			/* Render */
			this._t(__v0, this.renderWidget(this.dialog));
...
Object.assign(Runtime.Widget.Gallery.Gallery,
{
	components: function()
	{
		return Runtime.Vector.from(["Runtime.Widget.Gallery.GalleryDialog"]);
	},
	css: function(vars)
	{
		var res = "";
...
Runtime.Widget.Gallery.GalleryDialog = {
	...
	methods:
	{
		renderDialogContainer: function()
		{
			...
			/* Element 'div' */
			let __v0 = this._e(__v, "div", {"onClick":this.onContainerClick,"class":this._class_name(["widget_dialog__container--photo", ((this.model.big_image_contain) ? ("widget_dialog__container--photo_contain") : (""))])});
...
Runtime.Widget.Gallery.GalleryDialogModel.prototype = Object.create(Runtime.Widget.Dialog.BaseDialogModel.prototype);
...
Object.assign(Runtime.Widget.Gallery.GalleryDialogModel.prototype,
{
	...
	{
		var item = this.items.get(this.current);
		if (!item)
		{
			return "";
		}
	...
	_init: function()
	{
		...
		this.items = Runtime.Vector.from([]);
		this.big_image_contain = false;
		this.current = 0;
	},
});
Object.assign(Runtime.Widget.Gallery.GalleryDialogModel, Runtime.Widget.Dialog.BaseDialogModel);
...
Runtime.Widget.Tab.Tab = {
	...
	methods:
	{
		render: function()
		{
			...
			if (this.model.canShow(this.name))
			{
				/* Element 'div' */
				let __v0 = this._e(__v, "div", {"data-tab":this.name,"class":this._class_name(["tabs__item", ((this.model.isActive(this.name)) ? ("tabs__item--active") : (""))])});
...
Runtime.Widget.Tab.Tabs = {
	...
	methods:
	{
		renderHeader: function()
		{
			...
			{
				let tab = this.model.tabs.get(i);
				let tab_name = Runtime.rtl.attr(tab, "name");
				...
				{
					/* Element 'div' */
					let __v0 = this._e(__v, "div", {"data-name":tab_name,"onClick":this.onClick,"class":this._class_name(["tabs__header_item", ((is_active) ? ("tabs__header_item--active") : (""))])});
				...
				{
					/* Element 'a' */
					let __v1 = this._e(__v, "a", {"data-name":tab_name,"href":tab_href,"class":this._class_name(["tabs__header_item", ((is_active) ? ("tabs__header_item--active") : (""))])});
		...
		renderContent: function()
		{
			...
			if (this.model.render_widgets)
			{
				for (let i = 0; i < this.model.tabs.count(); i++)
				{
					let tab = this.model.tabs.get(i);
					let tab_name = Runtime.rtl.attr(tab, "name");
					let tab_model = Runtime.rtl.attr(tab, "model");
					
					if (tab_model)
					{
						/* Component 'Tab' */
						let __v0 = this._c(__v, "Runtime.Widget.Tab.Tab", {"name":tab_name,"model":this._model(this.model)}, () => {
							let __v = [];
							
							/* Render */
							this._t(__v, this.renderWidget(tab_model));
		...
		render: function()
		{
			...
			/* Element 'div' */
			let __v0 = this._e(__v, "div", {"class":this._class_name(["tabs"])});
		...
		onClick: function(e)
		{
			var tab_name = e.target.getAttribute("data-name");
...
Object.assign(Runtime.Widget.Tab.TabsModel.prototype,
{
	...
	initParams: function(params)
	{
		...
		if (params.has("tabs"))
		{
			this.tabs = params.get("tabs");
		}
		if (params.has("render_widgets"))
		{
			this.render_widgets = params.get("render_widgets");
		}
	},
	/**
	 * Returns true if active
	 */
	isActive: function(tab_name)
	{
	 ...
	 * Can show
	 */
	canShow: function(tab_name)
	{
		var tab = this.tabs.findItem(Runtime.lib.equalAttr("name", tab_name));
		if (tab == null)
		{
			return false;
		}
		if (tab.has("href") && tab.get("name") != tab_name)
		{
			return false;
		}
		return true;
...
Object.assign(Runtime.Widget.Table.DeleteDialogModel.prototype,
{
	...
	{
		this.title = this.table.getTitle(Runtime.Map.from({"action":"delete","item":item}));
		this.form.clear();
	...
	confirm: async function()
	{
		var res = await this.form.submit();
		if (res.isSuccess())
		{
			return Promise.resolve(true);
		}
...
Object.assign(Runtime.Widget.Table.SaveDialogModel.prototype,
{
	...
	{
		this.action = "add";
		this.title = this.table.getTitle(Runtime.Map.from({"action":"add"}));
		this.add_form.clear();
		/* Change confirm button */
		var confirm_button = this.buttons.getWidget("confirm_button");
	...
	showEdit: function(item)
	{
		...
		this.action = "edit";
		this.title = this.table.getTitle(Runtime.Map.from({"action":"edit","item":item}));
...
Runtime.Widget.Table.Table = {
	...
	methods:
	{
		...
		renderField: function(item, row_number, field)
		{
			...
			let field_model = field.get("model", null);
			let value = this.model.getItemValue(item, field_name);
			...
			{
				/* Render */
				this._t(__v0, this.model.limit * (this.model.page - 1) + row_number + 1);
			}
			else if (field_component != null)
			{
				...
				/* Component '{field_component}' */
				let __v1 = this._c(__v0, field_component, this._merge_attrs({"model":this._model(field_model),"value":value,"data":Runtime.Map.from({"item":item,"field_name":field_name,"row_number":row_number,"table":this.model})}, props));
			}
			else if (field_model != null)
			{
				/* Render */
				this._t(__v0, this.renderWidget(field_model, Runtime.Map.from({"value":value,"data":Runtime.Map.from({"item":item,"field_name":field_name,"row_number":row_number,"table":this.model})})));
			}
			else
			{
				/* Render */
		...
		renderPagination: function()
		{
			...
			if (fields && this.model.pages > 1)
			{
				...
				let __v2 = this._c(__v1, pagination_class_name, this._merge_attrs({"page":this.model.page,"pages":this.model.pages,"onPage":this.onChangePage}, props));
			}
 ...
 */
		...
		{
			return this.row_selected == row_number;
		},
		/**
 ...
 */
		...
		{
			return this.model.getItemByRowNumber(this.row_selected);
		},
		/**
 * OnRowClick
 */
...
Object.assign(Runtime.Widget.Table.TableRowButtonsModel.prototype,
{
	...
	initWidget: function(params)
	{
		...
		if (params.has("edit"))
		{
			show_edit = params.has("edit");
		}
		if (params.has("delete"))
		{
			show_delete = params.has("delete");
		}
		/* Edit button */
		if (show_edit)
		{
...
Object.assign(Runtime.Widget.Tree.TreeModel.prototype,
{
	...
	{
		var result = new Runtime.Reference(true);
		this.emit(new Runtime.Widget.Tree.TreeMessage(Runtime.Map.from({"name":"canDrag","src":src,"dest":dest,"kind":kind,"result":result})));
		return result.value();
	},
	/**
	 * Drag & Drop
 ...
 */
		checkInside: function(elem)
		{
			...
			var arr = Runtime.rs.split(".", path);
			arr.pop();
			while (arr.count() != 0)
			{
				path = Runtime.rs.join(".", arr);
 ...
 */
		setDragDestElement: function(elem, item, kind)
		{
			...
			{
				var can_drag = this.model.canDrag(this.getSrc(), this.getDest(), this.drag_dest_kind);
				if (!can_drag)
				{
					this.drag_dest_box = null;
				}
